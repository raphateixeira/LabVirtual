...
def MMA(x,t):                           # Define o nome da função que modela o sistema;
  ...
  dx = np.array([x1, x2])               # Derivada do vetor de estados
...
```{python}
T = 60                        # Tempo total de simulação
Ts = 0.1                      # Período de amostragem (passo de integração)
t = Ts*np.arange(0,T+Ts, Ts)
x0 = [0.1, -0.5]

# Integração numérica: odeint integra a função MMA com condição inicial x0 durante o tempo t.
x = odeint(MMA, x0, t)
```
